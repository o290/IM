...
const HeartbeatInterval = 10 * time.Hour // 心跳间隔
const HeartbeatTimeout = 30 * time.Hour  // 心跳超时时间

type MsgServerAck struct {
	Code   int   `json:"code"`   // 状态码（0=成功，非0=失败）
	MsgID  uint  `json:"msgID"`  // 关联的消息ID
	Status int32 `json:"status"` // 消息状态（1=已发送至服务端）
}
...
func ChatHandler(svcCtx *svc.ServiceContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			//msg:A
			if request.Msg.Type != ctype.ReadMsgType {
				msgServerID := uint(time.Now().UnixNano() / 1e6)
				serverAck := MsgServerAck{
					Code:   http.StatusOK,
					MsgID:  msgServerID,
					Status: MsgStatusSent, // 状态值对应“服务端已接收”
				}
				if err := conn.WriteJSON(serverAck); err != nil { // 使用 WriteJSON 发送结构化响应
					logx.Error("Send msg:A failed:", err)
				}
			}
			// 检验聊天对象是否是好友
			...
			// 判断type
			if !(request.Msg.Type >= 1 && request.Msg.Type <= 15) {
...
func SendMsgByUser(svcCtx *svc.ServiceContext, revUserId uint, sendUserID uint, msg ctype.Msg, msgID uint) {
	...
	if ok2 {
		...
		// 发送 ACK - 消息已发送
		//sendAck(svcCtx, msgID, sendUserID, revUserId, MsgStatusSent)
	}
